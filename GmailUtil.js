...
class GmailUtil {
  ...
  constructor({userId = 'me', initialQuery = ''}) {
    ...
    this.labels = new Labels(this.labels.labels.map(label => new Label(label)));
    this.threads = new Threads(this.threads.map(thread => new Thread(thread)));
    this.threads.getThreadsByRootLabel = label => {
      return this.labels.getLabelsByRootLabel(label).flatMap(label => this.threads.getThreadsByLabel(label));
    };
  }
}
...
class Labels {
  ...
  getLabelByName(labelName) {
    return this.getLabels().find(label => label.getName() === labelName);
  }

  getLabelsByRootLabel(label) {
    const labelName = label.getName();
    return this.getLabels().filter(
      label => label.getName() === labelName || label.getName().startsWith(`${labelName}/`)
    );
  }
}
...
class Thread {
  constructor({id, messages = []}) {
    this.id = id;
    this.messages = new Messages(messages.map(message => new Message(message)));
  }
...
class Threads {
  ...
  getThreadsByLabel(label) {
    ...
    return this.getThreads().filter(thread =>
      thread.messages.getMessages().some(message => message.getLabelIds().includes(labelId))
    );
  }
}
